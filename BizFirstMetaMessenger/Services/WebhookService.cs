...
public class WebhookService : IWebhookService
{
    ...
    public async Task ProcessWebhookEventAsync(WebhookEventRequest webhookEvent)
    {
        ...
        // Save raw webhook to database - ALWAYS save to get a valid ID
        var rawBody = webhookEvent.RawBody ?? "{}";
        var webhookEventId = await _databaseService.SaveWebhookEventAsync("webhook", rawBody, webhookEvent.Object);

        _logger.LogInformation("✓ Webhook event saved to database with ID: {WebhookEventId}", webhookEventId);
